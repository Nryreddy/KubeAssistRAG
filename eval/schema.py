...
Defines the Pydantic data models for the golden evaluation dataset.
Validates the structure of the seed questions YAML file and ensures uniqueness
and correctness of the evaluation inputs.
"""
...
def load_goldens(path: str | Path) -> list[Golden]:
    ...
    try:
        ...
        import sys
        print(f"Error: Golden questions file not found at {path}", file=sys.stderr)
        return []
    except yaml.YAMLError as exc:
        import sys
        print(f"Error: Failed to parse YAML at {path}:\n{exc}", file=sys.stderr)
        return []
