...
Core orchestration logic for the Retrieval-Augmented Generation pipeline.
Handles query routing, retrieval (dense, sparse, hybrid, HyDE), CRAG evaluation,
Self-Reflective generation, and inline SQL execution.
"""
...
def _generate(
    ...
    def llm_fn(prompt: str, last_error: str) -> str:
        return generate(system, prompt)["text"]

    try:
        chat_resp = validate_with_retry(raw, llm_fn)
    except Exception as exc:
        logger.exception("Failed to parse LLM response into JSON")
        # Fallback to returning raw string if we fail completely
    ...
    chat_resp.metadata = ResponseMetadata(
        route="rag",
