...
Handles generation of SQL queries from natural language and the safe execution
of those queries against the configured PostgreSQL database.
"""
import datetime
import decimal
import json
...
class SQLService:
    ...
    def execute_sql(self, sql: str) -> list[dict]:
        ...
        conn = psycopg2.connect(settings.database_url)
        # Enforce read-only transaction for safety against LLM hallucinations
        conn.set_session(readonly=True, autocommit=True)
        cur = conn.cursor()
        
        # Enforce a 10 second timeout on statements
