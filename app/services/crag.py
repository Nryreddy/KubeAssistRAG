...
Implements document relevance grading to assess if retrieved context is sufficient.
If the context is irrelevant or ambiguous, it triggers a fallback web search.
"""
import json
import logging
...
def should_trigger_web_search(evaluation: CRAGEvaluation) -> bool:
    """Return True if web search fallback should be triggered based on relevance score."""
    return (
...
def crag_pipeline(
    ...
    Evaluates retrieved chunks and falls back to web search if necessary.
    
    Args:
        question: User query.
        chunks: Initial retrieved chunks from the vector store.
        enable_crag: Feature flag to toggle CRAG evaluation.
        
    Returns:
        A tuple of (final_chunks, evaluation_result, did_web_search).
    """
