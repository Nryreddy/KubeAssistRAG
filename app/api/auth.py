...
It handles user credentials securely and issues JWT access tokens.
"""
import psycopg2
...
def _get_db_conn():
    """Establish a synchronous database connection using psycopg2."""
...
async def register(request: Request, body: dict) -> dict:
    ...
    Args:
        request: The raw FastAPI request to extract the client IP for rate limiting.
        body: JSON payload containing 'username' and 'password'.
        
    Returns:
        A dictionary containing the generated JWT 'token' on success.
        
    Raises:
        HTTPException: 429 on rate limit, 400 on bad input, 409 if user exists.
    """
...
async def login(request: Request, body: dict) -> dict:
    ...
    Returns:
        A dictionary containing the generated JWT 'token'.
        
    Raises:
        HTTPException: 429 on rate limit, 400 on bad input, 401 on invalid credentials.
    """
    client_ip = request.client.host if request.client else "unknown"
    allowed, _, _ = is_allowed_ip(
        client_ip,
